...
int main(int argc, char **argv) {
    ...
    open_spiel::algorithms::torch_az::VPNetModel *model = 
        new open_spiel::algorithms::torch_az::VPNetModel(*game, path, graph_def, "cuda:0");
    model->LoadCheckpoint(path.append("checkpoint-1500"));
    // open_spiel::algorithms::torch_az::UCTvUCT(game, 2048, "test_game.txt");
    for (int b = 16; b < 2048; b *= 2) {
        open_spiel::algorithms::torch_az::evaluatePUCT(game, b, 1500, 100, *model);
        open_spiel::algorithms::torch_az::evaluateLTS(game, b, 1500, 100, *model);
        open_spiel::algorithms::torch_az::evaluateBFLTS(game, b, 1500, 100, *model);
    }
}
